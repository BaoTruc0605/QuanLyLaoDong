...
public class TimKiemTheoMaNhanVien extends JFrame implements ActionListener, MouseListener, WindowListener {
	...
	public TimKiemTheoMaNhanVien() {
		...
		setTitle("Tìm nhân viên theo mã nhân viên");
		setSize(1030, 600);
		setDefaultCloseOperation(EXIT_ON_CLOSE);
		setLocationRelativeTo(null);
		NhanVien_DAO = new NhanVien_DAO();

		JLabel lblTieuDe = new JLabel("TÌM NHÂN VIÊN THEO MÃ NHÂN VIÊN");
		...
		btTim = new JButton("Tìm theo mã nhân viên");
	...
	public TimKiemTheoMaNhanVien(String tenDangNhap) throws HeadlessException {
		super();
	...
	public void actionPerformed(ActionEvent e) {
		if(e.getSource().equals(btTim)) {
			if(txtTimKiem.getText().equals("")) {
				...
				NhanVien nv = null;
				nv = NhanVien_DAO.getNhanVienTheoMa(maNV);
				if (nv == null) {
					JOptionPane.showMessageDialog(this, "Không tìm thấy nhân viên có mã này");
				}
				else {
					model.setRowCount(0);
					model.addRow(new Object[] { nv.getMaNhanVien(), nv.getHo(), nv.getTen(), nv.getNgaySinh(),
						nv.getGioiTinh() ? "Nam" : "Nữ", nv.getDiaChiThuongTru(), nv.getDiaChiTamTru(),
						nv.getSoDienThoai(), nv.getChuyenMon().getMaChuyenMon() + 
						"-" + nv.getChuyenMon().getTenChuyenMon(),
